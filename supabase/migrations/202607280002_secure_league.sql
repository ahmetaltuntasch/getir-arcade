-- Atomically accepts one validated run and updates all derived league data.
...
begin
  ...
  v_xp := least(400, case when p_game_id='getir-rush'
    then 50+coalesce((p_metrics->>'deliveries')::integer,0)*5+floor(p_score/100.0)::integer
    else 50+coalesce((p_metrics->>'rowsCleared')::integer,0)*15+floor(p_score/100.0)::integer end);
...
drop view if exists public.season_leaderboard,public.rush_leaderboard,public.depo_leaderboard;
create view public.season_leaderboard with(security_invoker=false) as
select row_number() over(order by sum(x.amount) desc,p.nickname asc)::integer rank,p.nickname,sum(x.amount)::bigint xp
from public.profiles p join public.xp_ledger x on x.user_id=p.id and x.created_at>=date_trunc('month',now() at time zone 'Europe/Istanbul')
group by p.id,p.nickname;
create view public.rush_leaderboard with(security_invoker=false) as
select row_number() over(order by max(r.score) desc,p.nickname asc)::integer rank,p.nickname,max(r.score)::bigint score
from public.profiles p join public.runs r on r.user_id=p.id and r.game_id='getir-rush' group by p.id,p.nickname;
create view public.depo_leaderboard with(security_invoker=false) as
select row_number() over(order by max(r.score) desc,p.nickname asc)::integer rank,p.nickname,max(r.score)::bigint score
from public.profiles p join public.runs r on r.user_id=p.id and r.game_id='depo-tetris' group by p.id,p.nickname;
