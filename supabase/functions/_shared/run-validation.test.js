...
test('accepts a plausible nebula escape',()=>assert.equal(validateRun({clientRunId:'nebula-12345678',gameId:'nebula-escape',score:8200,durationSeconds:90,completedAt:new Date(now).toISOString(),metrics:{threats:52,bestCombo:14,jumps:2,reason:'time'}},now).ok,true));
test('rejects a nebula combo above its threat count',()=>assert.equal(validateRun({clientRunId:'nebula-12345678',gameId:'nebula-escape',score:8200,durationSeconds:90,completedAt:new Date(now).toISOString(),metrics:{threats:12,bestCombo:14,jumps:2,reason:'time'}},now).ok,false));
