...
test('accepts a plausible televole broadcast',()=>assert.equal(validateRun({clientRunId:'televole-12345678',gameId:'televole-wars',score:8200,durationSeconds:90,completedAt:new Date(now).toISOString(),metrics:{headlines:52,bestCombo:14,bursts:2,reason:'time'}},now).ok,true));
test('rejects a televole combo above its headline count',()=>assert.equal(validateRun({clientRunId:'televole-12345678',gameId:'televole-wars',score:8200,durationSeconds:90,completedAt:new Date(now).toISOString(),metrics:{headlines:12,bestCombo:14,bursts:2,reason:'time'}},now).ok,false));
