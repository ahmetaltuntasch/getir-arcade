...
export const ENEMY_TYPES = {
  rumor: { hp: 1, speed: 78, radius: 16, damage: 9, score: 80, color: '#ff5aa5' },
  paparazzi: { hp: 2, speed: 62, radius: 20, damage: 13, score: 120, color: '#ffd300' },
  troll: { hp: 3, speed: 48, radius: 24, damage: 18, score: 180, color: '#8e71ff' },
  finale: { hp: 12, speed: 38, radius: 36, damage: 26, score: 900, color: '#33e6c4' },
};
...
export function enemyTypeFor(elapsed, roll = Math.random()) {
  if (elapsed > 74 && roll < .035) return 'finale';
  if (elapsed > 42 && roll < .23) return 'troll';
  if (elapsed > 16 && roll < .48) return 'paparazzi';
  return 'rumor';
}
...
export function resultXp({ score = 0, headlines = 0 } = {}) {
  return Math.min(400, 50 + Math.max(0, headlines) * 3 + Math.floor(Math.max(0, score) / 100));
}
