...
export class NebulaEscape {
  ...
  triggerBurst() {
    ...
    this.charge = 0; this.jumps++; this.primeUntil = this.elapsed + 6;
  ...
  update(dt) {
    ...
    this.events.hud?.({ time: formatTime(timeLeft), score: this.score, energy: p.energy, threats: this.threats, combo: this.combo, charge: this.charge, wave: d.wave, hyper: this.elapsed < this.primeUntil });
  }
  ...
  fire() {
    ...
    this.events.sound?.('laser');
  }
  defeat(enemy) {
    enemy.dead = true; this.threats++;
    ...
    const hyper = this.elapsed < this.primeUntil;
    this.score += calculateKillScore(enemy.type, this.combo, hyper);
    this.charge = Math.min(1, this.charge + (enemy.type === 'overlord' ? .32 : .045));
    this.burstParticles(enemy.x, enemy.y, enemy.color, enemy.type === 'overlord' ? 28 : 9);
    this.events.sound?.(enemy.type === 'overlord' ? 'boss' : 'pop');
  }
  burstParticles(x, y, color, count) { for (let i = 0; i < count; i++) { const a = Math.random() * TAU, speed = 45 + Math.random() * 130; this.particles.push({ x, y, vx: Math.cos(a) * speed, vy: Math.sin(a) * speed, color, life: .35 + Math.random() * .45, size: 2 + Math.random() * 5 }); } }
    ...
    this.events.end?.({ score: this.score, threats: this.threats, bestCombo: this.bestCombo, jumps: this.jumps, durationSeconds: Math.max(1, Math.min(RUN_DURATION, Math.round(this.elapsed))), reason, xp: resultXp({ score: this.score, threats: this.threats }) });
  }
  draw() {
    const c = this.ctx, p = this.player; c.clearRect(0, 0, WORLD.width, WORLD.height);
    const bg = c.createRadialGradient(470, 300, 20, 470, 300, 720); bg.addColorStop(0, '#4d1b76'); bg.addColorStop(.48, '#24104b'); bg.addColorStop(1, '#09051d'); c.fillStyle = bg; c.fillRect(0, 0, WORLD.width, WORLD.height);
    c.save(); c.fillStyle = '#fff'; for(let i=0;i<70;i++){const x=(i*137)%960,y=(i*83)%600,r=(i%5===0?1.8:.8);c.globalAlpha=.18+(i%7)/12;c.beginPath();c.arc(x,y,r,0,TAU);c.fill();} c.restore();
    c.save(); c.globalAlpha = .12; c.strokeStyle = '#bca7ff'; c.lineWidth = 2;
    ...
    this.spotlight(170, 80, '#ff4f9a'); this.spotlight(790, 90, '#4ee5c4');
    c.fillStyle = '#ffffff09'; c.beginPath(); c.ellipse(480, 326, 220, 110, 0, 0, TAU); c.fill(); c.strokeStyle = '#a88cff38'; c.lineWidth = 3; c.stroke();
  ...
  drawPlayer(p) {
    ...
    c.rotate(-.55); c.fillStyle = '#e9faff'; c.beginPath(); c.moveTo(15,-8);c.lineTo(46,0);c.lineTo(15,8);c.closePath();c.fill(); c.fillStyle = '#4ee5c4'; c.beginPath(); c.arc(43, 0, 7, 0, TAU); c.fill(); c.restore();
  }
  ...
  drawEnemy(enemy) {
    ...
    if (enemy.type === 'drone') { c.beginPath(); c.moveTo(0,-18);c.lineTo(18,10);c.lineTo(0,16);c.lineTo(-18,10);c.closePath();c.fill();c.stroke();c.fillStyle='#fff';c.beginPath();c.arc(0,2,4,0,TAU);c.fill(); }
    else if (enemy.type === 'comet') { c.beginPath(); c.arc(5, 0, 18, 0, TAU); c.fill(); c.stroke(); c.fillStyle='#fff3a5';c.beginPath();c.moveTo(-12,-11);c.lineTo(-34,-18);c.lineTo(-18,-3);c.lineTo(-38,8);c.lineTo(-13,10);c.closePath();c.fill(); }
    else if (enemy.type === 'hunter') { c.beginPath(); for(let i=0;i<6;i++){const a=-Math.PI/2+i*TAU/6;c.lineTo(Math.cos(a)*24,Math.sin(a)*24)} c.closePath();c.fill();c.stroke();c.fillStyle='#2b1246';c.beginPath();c.arc(0,0,9,0,TAU);c.fill();c.fillStyle='#fff';c.beginPath();c.arc(0,0,3,0,TAU);c.fill(); }
    else { c.beginPath(); for (let i = 0; i < 12; i++) { const a = i * Math.PI / 6, r = i % 2 ? 28 : 39; c.lineTo(Math.cos(a) * r, Math.sin(a) * r); } c.closePath(); c.fill(); c.stroke(); c.fillStyle = '#28134a'; c.font = '900 13px sans-serif'; c.textAlign = 'center'; c.fillText('OMEGA', 0, 5); }
